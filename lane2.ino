...
#include <WiFi.h>
#include <Wire.h>
#include <esp_bt.h>      // for esp_bt_controller_disable() / esp_bt_mem_release()
#include "IST8310.h"
// NOTE: BluetoothSerial intentionally removed.
// Bluetooth Classic (BR/EDR) and WiFi-AP share the single ESP32 radio via
// time-multiplexing. When a BT client was connected the radio coexistence
// disrupted TCP ACK / keepalive timing, causing the Jetson to see repeated
// "ESP not connected — peer reconnecting" drops. All debug output now goes
// to Serial (USB) only; the Jetson log already captures the Jetson side.
// esp_bt_controller_disable() + esp_bt_mem_release() in setup() free the BT
// controller entirely, eliminating the interference and recovering ~80 KB heap.

// ─── Serial-only logging (drop-in replacements for the old BLOG* macros) ───
#define BLOG(msg)      Serial.print(msg)
#define BLOGln(msg)    Serial.println(msg)
...
void enableI2C() {
    if (compassReady && i2cIsDisabled) {
        ...
        // Give the IST8310 100ms to settle on the bus after motor-EMI.
        // Without this, the first readHeading call hits a still-noisy I2C bus
        // and can freeze the ESP loop long enough to drop the WiFi TCP socket.
        delay(100);
        i2cIsDisabled = false;
    }
}
...
float readHeadingDeg() {
    ...
    // Hard wall-clock guard: if ist8310.update() internally retries Wire calls
    // (each capped at 50ms by Wire.setTimeOut) it can burn 20s total during an
    // I2C lockup caused by motor EMI. Bail out if the entire update takes >100ms
    // so the loop() — and therefore TCP handling — is never starved for longer.
    unsigned long t0 = millis();
    bool ok = ist8310.update();
    if ((millis() - t0) > 30UL) {
        BLOGf("[Compass] WARNING: ist8310.update() took %lums — I2C may be locked\r\n",
              millis() - t0);
        return -1.0f;   // treat as a read failure; caller will use recoverI2C()
    }
    if (!ok) return -1.0f;
...
void setup() {
    ...
    // Release the Bluetooth controller and its memory before starting WiFi.
    // This is the single most important step for WiFi-AP stability on ESP32:
    // Bluetooth Classic (BR/EDR) and WiFi share one radio and one set of
    // antenna switch resources. Even when no BT client is connected the BT
    // controller's presence causes periodic radio arbitration events that
    // disrupt TCP session timing. Freeing it here guarantees the radio is
    // 100% dedicated to WiFi for the rest of the session.
    esp_bt_controller_disable();
    esp_bt_mem_release(ESP_BT_MODE_BTDM);
    Serial.println("[BT] Controller disabled & memory released — WiFi-only mode");
    ...
    BLOGln("[WiFi] Ready. Connect Jetson to SSID WaterBot and TCP 192.168.4.1:4210");
}
...
void loop() {
    ...
    // ── 3. Manage client connections & TCP Disconnect Safety Watchdog ──
    if (!client || !client.connected()) {
        // CRITICAL SAFETY FIX: If TCP disconnects while bot is moving/turning,
        // IMMEDIATELY stop motors so the boat does not keep turning endlessly!
        if (isMoving || isCompassTurning || isCMoveDriving) {
            BLOGln("[SAFETY] TCP disconnected during movement! Emergency stopping motors.");
            cancelMotionSilent();
        }
