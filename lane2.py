...
ESP_RECONNECT_WAIT_S   = 30.0   # max wait for ESP reconnect before dry-running a command
...
# If depth is N/A (cov too low) for this long during a scan-dance settle
# or GETHEADING wait, move on rather than blocking indefinitely.
DEPTH_NA_SCAN_TIMEOUT_S = 5.0   # was: waited forever
...
SEARCH_LANE_SHIFT_M    = 0.7    # m   — sideways shift between lanes
...
# of hardcoding it.
COLLECT_MAX_DIST_M     = 1.0   # m — 0.5m: collect if <=, else ignore
...
DIST_CACHE_NAV_MAX_AGE_S = 3.0   # s — max cache age allowed for NAV DISPATCH.
                              #       Cached depth older than this is NOT trusted to
                              #       trigger navigate() — the object may have moved or
                              #       been collected, making the cached distance stale.
                              #       Must be < DIST_CACHE_GRACE_S (5s) so stabilise()
                              #       can still use the cache during approach turns.
DIST_NA_SUSTAINED_S  = 3.0    # s  — if depth has been N/A THIS long continuously
                              #       (per track), stop using the cached value for nav
                              #       dispatch and drop the entry from sorted_targets.
                              #       Prevents false movements from persistently N/A objects.
...
class ESPCommander:
    ...
    def get_heading(self, timeout=2.0):
        with self._cmd_lock:
            while not self._connected and not self._stop.is_set():
                print("[ESP] Connection lost — waiting for reconnect to read heading...")
                time.sleep(1.0)
            ...
            print("[ESP] WARNING: timeout waiting HEADING for CMD:GETHEADING — forcing reconnect")
            self._close_socket()
            return None
    ...
    def _send_and_wait(self, cmd, timeout=DONE_TIMEOUT_S,
        ...
        with self._cmd_lock:
            # Block until TCP is re-established before sending
            while not self._connected and not self._stop.is_set():
                print(f"[ESP] Connection lost. Pausing execution — waiting to reconnect before: '{cmd}'...")
                time.sleep(1.0)
            ...
            while time.time() < deadline:
                ...
                # If connection drops mid-command, hold until reconnected and resend
                if not self._connected:
                    print(f"[ESP] Connection dropped mid-wait for '{cmd}'. Waiting for reconnect...")
                    while not self._connected and not self._stop.is_set():
                        time.sleep(1.0)
                    if self._stop.is_set():
                        return False
                    print(f"[ESP] Reconnected! Resending command: '{cmd}'")
                    self._done_ev.clear()
                    self._send_line(cmd)
                    deadline = time.time() + timeout  # Reset timeout window
            ...
            print(f"[ESP] WARNING: timeout waiting DONE after '{cmd}' — forcing reconnect")
            self._close_socket()
            return False
    ...
    def _send_line(self, cmd):
        ...
        if sock:
            ...
            print(f"[ESP] Socket disconnected — cannot send '{cmd}'")
...
def main():
    ...
    while True:
            ...
            for box in result_l.boxes:
                ...
                if tid not in _dist_cache:
                    _dist_cache[tid] = {
                        ...
                        'na_since': 0.0,   # timestamp of first consecutive depth-N/A
                    }
                ...
                if dist is not None and cov >= DIST_MIN_COVERAGE:
                    ...
                    # Track how long depth has been N/A consecutively for this ID.
                    if _cache.get('na_since', 0.0) == 0.0:
                        _cache['na_since'] = now_loop   # first N/A frame in this streak
                    _na_duration = now_loop - _cache['na_since']

                    age = now_loop - _cache['ts']
                    if _cache['dist'] > 0 and age < DIST_CACHE_GRACE_S:
                        # Sustained N/A guard: if depth has been N/A for longer than
                        # DIST_NA_SUSTAINED_S, the cached value is too stale to trust
                        # for nav dispatch (object may have moved / been collected).
                        # Drop from sorted_targets so navigate() is NOT triggered.
                        # stabilise() can still use the tracker entry if it already
                        # started approach (it has its own fallback logic).
                        if _na_duration > DIST_NA_SUSTAINED_S:
                            print(f"  [depth] ID={tid} depth N/A {_na_duration:.1f}s "
                                  f"(>{DIST_NA_SUSTAINED_S:.0f}s) — cached {_cache['dist']:.2f}m "
                                  f"too stale for nav, skipping")
                            depth_fail += 1
                            continue
                ...
                # Reset the N/A streak counter now that we have a valid dist.
                _cache['na_since'] = 0.0
        ...
        if sorted_targets:
            ...
            # No valid-depth target in this frame (depth_fail / id_fail).
            # Default to False so raw_hold_active alone does NOT fire the
            # interrupt and break an ongoing ESP compass turn for a target
            # that isn't confirmed to be in collect range.  The interrupt will
            # fire on the NEXT frame where depth resolves and the distance is
            # actually within COLLECT_MAX_DIST_M.
            _within_collect_range = False
        ...
        if not _nav_busy_snapshot and not _scan_settling and _effective_targets:
            ...
            if scan_active:
                ...
                def navigate(tid=tid):
                    ...
                    nonlocal _anon_target, _post_scan_best_target

                    while not esp.connected:
                        print("[NAV] ESP disconnected — holding navigation until TCP reconnects...")
                        time.sleep(1.0)
                    ...
                    # ── STALE-FRAME FLUSH ──────────────────────────────────
                    # After reversing, the camera buffer still holds frames
                    # captured DURING the backward move when the collected
                    # bottle was still in view. Those stale frames can make
                    # _post_scan_best_target or _anon_target look like a live
                    # new target and trigger a duplicate collection. Discard
                    # both caches here and wait for a fresh YOLO frame
                    # (tracked via the RTSPReader frame-ID counter) before
                    # the main loop is allowed to dispatch navigate() again.
                    _post_scan_best_target = None
                    _anon_target           = None
                    # Drain: record the current frame ID and spin until the
                    # reader has delivered at least 3 NEW frames so the
                    # detection pipeline is guaranteed to be processing
                    # post-backward imagery.
                    _drain_start    = time.time()
                    _drain_frame_id = last_left_frame_id   # snapshot before drain
                    _fresh_needed   = 3
                    while time.time() - _drain_start < 3.0:
                        _cur_fid = rl._frame_id   # RTSPReader's monotonic counter
                        if _cur_fid >= _drain_frame_id + _fresh_needed:
                            break
                        time.sleep(0.05)
                    # ── END STALE-FRAME FLUSH ──────────────────────────────
                    ...
                    def search_pattern():
                        ...
                        def _sp_pause_for_collection(label):
                            ...
                            if nav_started:
                                ...
                                # FIX: after nav+collection finishes the collected bottle
                                # is gone, but nav_interrupt_event may still be SET for up
                                # to RAW_SEEN_HOLD_S (0.8s) because the main loop hasn't
                                # had a chance to re-evaluate and clear it yet.
                                # Wait for the main loop to clear it (up to 2s) so the
                                # first _sp_wait after we resume does NOT immediately see
                                # a stale set and re-trigger another spurious collection.
                                _stale_wait = time.time()
                                while nav_interrupt_event.is_set() and time.time() - _stale_wait < 2.0:
                                    time.sleep(0.05)
                                if nav_interrupt_event.is_set():
                                    # Still set after 2s — nothing new was collected, the
                                    # event is genuinely stale. Clear it so patrol resumes
                                    # normally. The main loop will re-set it immediately if
                                    # a real new target is actually visible.
                                    print("  [search] nav_interrupt_event still set after 2s grace — "
                                          "clearing stale interrupt before resuming patrol")
                                    nav_interrupt_event.clear()
                            else:
                                print("  [search] Navigation did not start (transient detection?), resuming search...")
                                time.sleep(1.0)
                                # Same stale-interrupt guard for the no-nav path: the main
                                # loop's RAW_SEEN_HOLD_S window may still be active even
                                # though no nav ever fired. Wait for it to expire.
                                _stale_wait = time.time()
                                while nav_interrupt_event.is_set() and time.time() - _stale_wait < 2.0:
                                    time.sleep(0.05)
                                if nav_interrupt_event.is_set():
                                    print("  [search] nav_interrupt_event still set (transient) — "
                                          "clearing stale interrupt before resuming patrol")
                                    nav_interrupt_event.clear()
                        ...
                        def _sp_wait(label="settle"):
                            ...
                            When nav_interrupt_event fires during the settle:
                            - If sorted_targets already has a valid in-range target → pause
                              for collection immediately.
                            - If sorted_targets is empty (depth N/A or out-of-range):
                              wait a short grace window (RAW_SEEN_HOLD_S + 0.2s) for depth
                              to resolve. If still empty after that → skip and resume scan.
                              This prevents the scan-dance from freezing when the camera
                              sees an object but stereo depth is unavailable or the object
                              is beyond COLLECT_MAX_DIST_M."""
                            while True:
                                ...
                                while time.time() < deadline:
                                    ...
                                    time.sleep(0.05)
                                if interrupted:
                                    # Quick check: does the interrupt have a real in-range target?
                                    if sorted_targets:
                                        # Valid depth + in-range — collect now.
                                        _sp_pause_for_collection(label)
                                        break
                                    # No valid target yet. Give depth a short window to resolve
                                    # (≈ RAW_SEEN_HOLD_S + a tiny margin) before giving up.
                                    _depth_grace = RAW_SEEN_HOLD_S + 0.2
                                    _grace_start  = time.time()
                                    _resolved     = False
                                    while time.time() - _grace_start < _depth_grace:
                                        if sorted_targets:
                                            _resolved = True
                                            break
                                        if not nav_interrupt_event.is_set():
                                            break   # object left ROI — nothing to wait for
                                        time.sleep(0.05)
                                    if _resolved:
                                        _sp_pause_for_collection(label)
                                    else:
                                        # Depth stayed N/A or object is out of range.
                                        # Resume scan-dance immediately — do NOT block here.
                                        print(f"  [scan-dance] depth N/A / out-of-range at "
                                              f"'{label}' — skipping, resuming scan")
                                    break   # always break; outer step decides what's next
                                else:
                                    break
                        ...
                        def _sp_turn_capped(angle_deg, label, tol_deg=None):
                            """Like _sp_turn but caps each individual sub-step at
                            SCAN_STEP_DEG (30°) so the bot never rotates more than
                            30° in one go. Larger angles are broken into multiple
                            30° increments with intermediate pauses/checks.
                            This prevents the 3rd scan-dance split from sometimes
                            being 40-50° when drift from the first two steps has
                            accumulated."""
                            remaining = angle_deg
                            sign      = 1 if angle_deg >= 0 else -1
                            step_idx  = 0
                            while abs(remaining) > 0.5:   # 0.5° is within compass tolerance
                                chunk = sign * min(SCAN_STEP_DEG, abs(remaining))
                                step_idx += 1
                                chunk_label = (f"{label} sub{step_idx}"
                                               if abs(angle_deg) > SCAN_STEP_DEG + 1.0
                                               else label)
                                _sp_turn(chunk, chunk_label, tol_deg=tol_deg)
                                remaining -= chunk
                        ...
                        def _scan_dance(label):
                            """Note the centre heading (cheading), then per side:
                              ...
                              - one precise absolute turn to cheading+-90 (edge),
                                broken into 30deg increments so no single step
                                exceeds 30deg and mid-stops are never skipped
                              - one precise turn back to cheading, verified

                            Depth N/A during a scan step: if the tracker reports
                            N/A depth for longer than 5 s we give up waiting and
                            ...
                            # ── Right side ─────────────────────────────────────
                            # Two coarse 30° splits (no reach-verify needed).
                            ...
                            _sp_turn(+SCAN_STEP_DEG, f"{label} right-step-2/3",
                                     tol_deg=SCAN_STEP_COARSE_TOL_DEG)
                            # Final turn to the absolute edge heading, broken into
                            # 30° chunks so the bot always makes at least one
                            # mid-stop regardless of how much drift accumulated.
                            if center_heading is not None:
                                cur_r = esp.get_heading()
                                if cur_r is not None:
                                    delta_r = heading_diff_deg(cur_r,
                                                               (center_heading + SCAN_EDGE_DEG) % 360.0)
                                    _sp_turn_capped(delta_r, f"{label} right-edge",
                                                    tol_deg=SCAN_EDGE_TOL_DEG)
                                else:
                                    _sp_turn(+SCAN_STEP_DEG, f"{label} right-step-3/3")
                            else:
                                _sp_turn(+SCAN_STEP_DEG, f"{label} right-step-3/3")
                            # Return to centre: ONE full single turn to the absolute
                            # center heading — no sub-steps, no intermediate stops.
                            if center_heading is not None:
                                if not _sp_turn_to(center_heading,
                                                   f"{label} return-to-center-from-right"):
                                    _sp_turn(-SCAN_EDGE_DEG,
                                             f"{label} return-to-center-from-right")
                            else:
                                _sp_turn(-SCAN_EDGE_DEG,
                                         f"{label} return-to-center-from-right")
                            ...
                            _sp_turn(-SCAN_STEP_DEG, f"{label} left-step-2/3",
                                     tol_deg=SCAN_STEP_COARSE_TOL_DEG)
                            if center_heading is not None:
                                cur_l = esp.get_heading()
                                if cur_l is not None:
                                    delta_l = heading_diff_deg(cur_l,
                                                               (center_heading - SCAN_EDGE_DEG) % 360.0)
                                    _sp_turn_capped(delta_l, f"{label} left-edge",
                                                    tol_deg=SCAN_EDGE_TOL_DEG)
                                else:
                            ...
                            if center_heading is not None:
                                if not _sp_turn_to(center_heading,
                                                   f"{label} return-to-center-from-left"):
                                    _sp_turn(+SCAN_EDGE_DEG,
                                             f"{label} return-to-center-from-left")
                            else:
                                _sp_turn(+SCAN_EDGE_DEG,
